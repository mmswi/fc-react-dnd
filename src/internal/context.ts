 ...
 *
 * Everything here is fixed for the provider's lifetime, and that stability is the floor the whole
 * architecture stands on: a changing context value writes `lanes` onto every consumer fiber and
 * walks up setting `childLanes` (`ReactFiberNewContext.js:245-254`), which is the exact
 * bookkeeping `bailoutOnAlreadyFinishedWork` re-checks — and `memo` cannot stop it. If this value
 * churns, no amount of `memo` on list items saves the drag (`ANALYSIS.md` § A3.5).
 *
 * Drag state is read through `useStoreSelector`, never through context.
 */
